...
export default function CarHero() {
  ...
  useEffect(() => {
    ...
    const trimCache = () => {
      const maxEntries = mode === 'mobile' ? 20 : 24
    ...
    const warmFrames = (target, direction) => {
      ...
      for (const offset of offsets) {
        ...
        const maxInFlight = mode === 'mobile' ? 6 : 8
    ...
    loadFrame(0, 'high').then(async () => {
      if (disposed) return
      await preloadRange(1, INTRO_END_FRAME, 4)
      if (disposed) return
      ...
      media.add(
        ...
        (context) => {
          ...
          timeline
            ...
            .to(cueRef.current, { autoAlpha: 0, duration: 8 }, 88)
